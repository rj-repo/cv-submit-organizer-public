package org.rj.application_service.it;
...
import org.rj.application_service.infrastructure.persistence.JpaApplicationJob;
...
class ApplicationJobResourceIntegrationTest extends TestContainersInitConfiguration {
    ...
    void getJobsForUser_expectOK() {
        ...
        ResponseEntity<UserJobApplications> voidResponseEntity = restTemplate.exchange(
                "http://localhost:" + port + "/api/v1/applications?page=10",
                HttpMethod.GET,
                new HttpEntity<>(headers),
                UserJobApplications.class);


        assertThat(voidResponseEntity.getStatusCode().value()).isEqualTo(200);
        UserJobApplications body = voidResponseEntity.getBody();
        assertNotNull(body);
